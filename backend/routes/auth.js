...
router.post('/createuser',[
 ...
 if(user){
   return res.status(400).json({errors:"sorry this email user alredy excist"})
 }
...
//ROUTE -2:  Authanticate  a User using:POST  "/api/auth/createuser". no login required 
router.post('/login',[
   ...
   if (!result.isEmpty()) {
    ...
    try {
      ...
      if(!user){
         return res.status(400).json({ errors:'plese enter the correct information'});
      }
